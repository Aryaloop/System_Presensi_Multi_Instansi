...
const PaketManager = lazy(() => import("./SuperAdminSections/PaketManager")); // <--- BARU
export default function DashboardSuperAdmin() {
  ...
  const renderContent = () => {
    switch (page) {
      ...
      case "paket": return <PaketManager />; // <--- BARU
      default:
        return <DashboardHome />;
    }
  };
      ...
      <aside className="w-64 bg-white border-r shadow-sm flex flex-col fixed h-full z-10">
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-2 rounded-md transition ${page === "paket" ? "bg-indigo-100 text-indigo-600 font-medium" : "hover:bg-gray-100 text-gray-700"}`}
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
